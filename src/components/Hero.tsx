...
function Hero() {
  ...
  // Suggested Searches
  const [suggestedSearches, setSuggestedSearches] = useState<Tags[] | null>(
    null,
  );
  ...
  const handleSignOut = async () => {
    try {
      ...
      //
      console.error(
        "Error in handleSignOut in Hero.",
        "Message : ",
        msg,
        "Error : ",
        err,
    ...
    >
      ...
      <section className="w-full text-center px-2">
            ...
            {(suggestedSearches || isFetchingSuggestion || searchedError) && (
              <div className="absolute w-full top-12 bg-background rounded-full text-center p-1 border-2 border-secondary">
                ...
                {suggestedSearches?.map((search, idx) => (
                  <button
                    onClick={() => handleSearchSuggestion(search.name)}
                    key={idx}
                    className="w-full space-y-2 text-gray-400 p-2 rounded-full cursor-pointer text-left hover:bg-gray-200 transition-colors"
                  >
        ...
        <p className="text-secondary my-2 md:text-md xxs:text-sm">
          ...
          <strong className="text-primary">
            {pathname === "/"
              ? "World"
              : pathname === "/top-country-places"
                ? urlParams.country
                : urlParams.city}
          </strong>{" "}
          ,{" "}
          <strong>
            {pathname === "/top-city-places" && urlParams.country}
          </strong>
        </p>
      </section>
      ...
      {status !== "authenticated" ? (
        <p className="text-primary font-semibold text-sm text-center">
          Login To Add Tag , Places and Votes
        </p>
      ) : (
        <button
          onClick={handleSignOut}
          className="text-primary underline cursor-pointer hover:text-secondary font-semibold"
        >
          Logout
        </button>
      )}
