...
interface AddPlaceSelectorProps {
  ...
  setSelectedPlaceDetails: React.Dispatch<
    React.SetStateAction<PlaceDetailsResponse | null>
  >;
}
...
const AddPlaceSelector = ({
  ...
  const [isFetchingSuggestion, setIsFetchingSuggestion] =
    useState<boolean>(false);
  // user searched place
  const [searchedPlace, setSearchedPlace] = useState<string>("");
  // Suggested place's
  const [suggestedPlaces, setSuggestedPlaces] = useState<
    PlaceSuggestionResponse[] | null
  >(null);
  // selected place
  const [selectedPlace, setSelectedPlace] =
    useState<PlaceSuggestionResponse | null>(null);
  ...
  const fetchPlaceSuggestion = async (value: string) => {
    if (value.length > 3) {
      ...
      try {
        ...
        //
        console.error(
          "Error in AddPlacePop in fetchPlaceSuggestion.",
          "Message : ",
          msg,
          "Error : ",
          err
  ...
  const handleConfirmation = async (e: React.FormEvent) => {
    ...
    try {
      ...
      //
      console.error(
        "Error in AddPlacePop in handleConfirmation.",
        "Message : ",
        msg,
        "Error : ",
        err
  ...
  return (
    <div>
      {/*  */}
      <h3 className="text-2xl text-secondary font-semibold">Add Place</h3>
      ...
      <div className="relative w-full">
        ...
        <input
          ...
          type="search"
          autoComplete="off"
          className="rounded-xl border-2 border-gray-400 w-full p-2 outline-none my-2 focus:border-primary transition-colors text-secondary"
            ...
            {suggestedPlaces?.map((place, idx) => {
              ...
              return (
                <button
                  ...
                  key={idx}
                  className="space-y-2 text-gray-400 p-2 cursor-pointer text-left border rounded-lg hover:bg-gray-200 transition-colors"
                >
          ...
          {!selectedPlace ? (
            <div className="m-4">
              ...
              <p className="text-[10px] text-gray-500">
                Click the suggested place to add.
              </p>
            </div>
          ) : (
            <div className="relative bg-gray-100 text-left p-2 space-y-1">
              ...
              <button
                ...
                type="button"
                className="bg-gray-300 p-1.5 shadow-sm absolute top-2 right-2 rounded-full cursor-pointer hover:bg-secondary/20  transition-colors"
              >
              ...
              <p className="text-sm text-secondary">
                {selectedPlace.description.split(",").slice(1).join(",").trim()}
              </p>
            </div>
          )}
        </div>
...
//
const AddPlaceConfirmation = ({
  selectedPlaceDetails,
  setSelectedPlaceDetails,
  setIsAddPlacePop,
}: AddPlaceConfirmationProps) => {
  // Provider
  // Loaction
  ...
  const fetchTagSuggestion = async (value: string) => {
    if (value.length > 2) {
      ...
      try {
        ...
        //
        console.error(
          "Error in AddPlacePop in fetchTagSuggestion.",
          "Message : ",
          msg,
          "Error : ",
          err
  ...
  const handlePlaceAdd = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/add/place`,
        {
          method: "POST",
          headers: {
      ...
      //
      console.error(
        "Error in AddPlacePop in handlePlaceAdd.",
        "Message : ",
        msg,
        "Error : ",
        err
  ...
  return (
    <div>
      ...
      <button
        onClick={() => handleBack()}
        className="bg-white shadow-sm rounded-full p-2 absolute left-3 top-2.5 hover:bg-secondary/20 transition-colors cursor-pointer"
      >
      ...
      <div className="bg-gray-100 border-2 border-gray-400 p-4 rounded-lg text-left space-y-1">
        ...
        <h4 className="font-semibold text-lg text-secondary">
          {selectedPlaceDetails.googleData.name}
        </h4>

        {/* Place fullladdress */}
        <p className="text-sm text-gray-500">
          {selectedPlaceDetails.googleData.formatted_address}
        </p>
        ...
        <div className="flex flex-row items-center gap-x-1">
          ...
          <p className="text-primary">
            {selectedPlaceDetails.googleData.rating}
          </p>
          <p className="text-gray-500 text-sm">
            ({selectedPlaceDetails.googleData.user_ratings_total})
          </p>
        </div>
        ...
        <div className="flex flex-row items-center flex-wrap gap-1 my-2">
          {selectedPlaceDetails.dbData &&
            selectedPlaceDetails.dbData.place_tag &&
        ...
        <div className="mt-2">
          ...
          <div className="w-full flex flex-col gap-y-1 my-2 relative">
            <label className="text-sm text-secondary" htmlFor="searchedTag">
              Add Custom Tag :
            </label>
            <input
              name="searchedTag"
              ...
              <div className="w-full max-h-50 bg-background absolute z-20 top-16 rounded-lg overflow-y-scroll flex flex-col gap-y-2 p-2 border-2 border-gray-500">
                ...
                {suggestedTags?.length === 0 && (
                  <p className="m-2 text-gray-500 ">
                    No tags found with this string.
                  </p>
                )}
            ...
            <div className="flex flex-row items-center flex-wrap gap-1 my-2 ">
              {selectedTags.length !== 0 ? (
                selectedTags.map((tag, idx) => (
                  <div
                    key={idx}
                    className="flex items-center rounded-full bg-secondary px-3 py-1 gap-x-2"
                  >
                    <p className="text-[13px] text-primary">{tag}</p>

                    <button
                      onClick={() => handleTagRemove(tag)}
                      className="rounded-full p-0.5 bg-primary"
                    >
                      <X className="w-3 h-3 text-secondary " />
                    </button>
                  </div>
      ...
      <button
        ...
        disabled={isAddingPlace || selectedTags.length === 0}
        className="w-full bg-primary text-white mt-2 rounded-lg py-2 font-semibold border-2 border-primary enabled:hover:bg-transparent enabled:hover:text-primary transition-colors enabled:cursor-pointer disabled:bg-primary/60"
      >
        {isAddingPlace ? (
          <Loader dotSize="3" className="my-2" />
        ) : (
          <p>Countinue</p>
        )}
      </button>
    ...
    <section className="fixed min-w-screen min-h-screen bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center px-2">
      <div className="w-11/12 sm:w-[96%] md:w-[28rem] bg-background rounded-lg shadow-sm px-2 py-4 border border-secondary/20 relative text-center overflow-hidden">
        ...
        <button
          ...
          type="button"
          className="p-2 bg-white shadow-sm absolute top-2 right-2 rounded-full cursor-pointer hover:bg-secondary/20 transition-colors"
        >
        ...
        {!selectedPlaceDetails ? (
          ...
          <AddPlaceConfirmation
            ...
            setSelectedPlaceDetails={setSelectedPlaceDetails}
            setIsAddPlacePop={setIsAddPlacePop}
          />
        )}
      </div>
    </section>
